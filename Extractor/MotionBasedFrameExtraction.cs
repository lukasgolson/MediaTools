...
public static class MotionBasedFrameExtraction
{
    ...
    {
        var frame1 = previousFrame.ToEmguBitmap().Convert<Gray, byte>();
        var frame2 = currentFrame.ToEmguBitmap().Convert<Gray, byte>();

        var flow = new Mat();

        CvInvoke.CalcOpticalFlowFarneback(frame1, frame2, flow, 0.5, 3, 15, 3, 5, 1.2, 0);


        var flowX = new Mat(frame1.Size, DepthType.Cv32F, 1);
        var flowY = new Mat(frame1.Size, DepthType.Cv32F, 1);
